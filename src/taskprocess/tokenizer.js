...
class Tokenizer{
	...
	makeTokens(task, log, time, useWeekday, useDaytime, force){
		...
		console.log('makeTokens', {time, useWeekday, useDaytime, force});
		console.log(_.pick(log, 'name', 'type', 'time'));
		...
		console.log(task.name, {weekdayIndex, daytime})
		if (useWeekday==false){
			weekdayIndex = -1;
		}
		if (useDaytime==false){
